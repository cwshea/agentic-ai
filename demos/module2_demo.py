...
applications, technology stacks, and GCP infrastructure requirements.
...
def section_1_framework_comparison() -> None:
    header("SECTION 1 — LangChain vs Google ADK", "cyan")
    box(
        "Framework Architecture Comparison",
        "Module 1 uses Google ADK. Module 2 uses LangChain + LangGraph.\n"
        "Both implement the same think-act-observe loop with different approaches.",
    )

    print("\n  MODULE 1 (Google ADK) — GCP Infrastructure Agent")
    code_block("""
...
runner = Runner(agent=agent, app_name="gcp_infra_agent",
...
model = ChatGoogleGenerativeAI(model_id="claude-sonnet-4", location="us-central1")
...
response = executor.invoke({"input": "Analyze repository at /path"})
    ...
    concept(
        "Both frameworks implement the same think-act-observe loop. "
        "The difference is in the orchestration layer: "
        "Google ADK provides a simpler API with Gemini, while LangChain offers "
        "more flexibility and integration with the broader ecosystem (LangGraph, LangSmith)."
    )
    pause()
...
and GCP infrastructure requirements.
...
- map_aws_services: Map dependencies to required GCP services
...
- Provide specific GCP service recommendations
...
def create_agent():
    ...
    concept(
        "Compare this to Module 1 (Google ADK) which uses the same pattern: "
...
def section_3_context_management() -> None:
    ...
    concept(
        "Module 1 uses Google ADK's InMemorySessionService to keep the last N "
    ...
    print("\n  Module 1 (Google ADK) Context Management:\n")
    code_block("""
from google.adk.sessions import InMemorySessionService
...
agent = Agent(
    ...
    conversation_manager=InMemorySessionService(window_size=10),
    #                                                      ^^^^^^^^^^^^
...
# The agent remembers the previous scan and can reference it
    ...
    concept(
        ...
        "conversations, or branch conversations. Module 1's session service is simpler "
        "but less flexible - it automatically manages a fixed-size window."
    )

    print("\n  Key Differences:\n")
    print("  Module 1 (Google ADK):")
...
model = ChatGoogleGenerativeAI(model_id="claude-sonnet-4")
...
print(result)  # "This appears to be a Node.js application with..."
    ...
    try:
        ...
        print(f"  [Note: Chain requires GCP credentials to run. Error: {type(e).__name__}]\n")

    concept(
        "Chains are deterministic and efficient. They're perfect for known workflows "
        "like 'analyze dependencies → map to GCP services → generate report'. "
...
def map_aws_services(dependencies: str) -> str:
    \"\"\"Map dependencies to required GCP services.\"\"\"
    pass
...
# ---------------------------------------------------------------------------
# Section 6 — Tools: LangChain vs Google ADK
# ---------------------------------------------------------------------------

def section_6_tools_comparison() -> None:
    header("SECTION 6 — Tools: LangChain vs Google ADK", "magenta")
    box(
        "Comparing tool patterns across frameworks",
        "Both LangChain and Google ADK use tools to extend agent capabilities, "
    ...
    print("\n  Module 1 (Google ADK) Tool Definition:\n")
    code_block("""
# module1/tools/gcp_tools.py

# Google ADK tools are plain Python functions

def list_aws_resources_impl(service: str, region: str = "us-central1") -> dict:
    \"\"\"Implementation function.\"\"\"
    if service == "ecs":
        return {"clusters": ["prod-cluster", "dev-cluster"]}
    # ... more logic

# Google ADK uses plain functions (no decorator needed)
list_aws_resources = Tool(
    name="list_aws_resources",
    description="List GCP resources for a specific service (Cloud Run, Compute, Cloud SQL, Cloud Functions)",
    func=list_aws_resources_impl,
    parameters={
        "service": {
            "type": "string",
            "description": "GCP service name (ecs, ec2, rds, lambda)",
            "required": True,
        },
        "region": {
            "type": "string", 
...
def scan_repository_structure(repo_path: str) -> str:
    ...
    concept(
        "LangChain uses decorators (@tool) for a Pythonic API. Google ADK uses plain "
    ...
    print("\n  Key Differences:\n")
    print("  ┌─────────────────────┬──────────────────────────┬──────────────────────────┐")
    print("  │ Aspect              │ Google ADK (Module 1)    │ LangChain (Module 2)     │")
    print("  ├─────────────────────┼──────────────────────────┼──────────────────────────┤")
    ...
    print("\n  Module 1 (Google ADK) - Passing Tools to Agent:\n")
    code_block("""
from google.adk.agents import Agent
...
agent = Agent(
    ...
    conversation_manager=InMemorySessionService(),
)
    """)
...
agent = create_react_agent(
    ...
    code_block("""
# Google ADK Tool Output (Python dict)
{
...
'{
    ...
    concept(
        ...
        "use Google ADK or LangChain, you're giving the agent the same capability - the ability "
    ...
    concept(
        "Module 1 uses 5 GCP tools (list, describe, health, summary, human review). "
...
def section_7_repository_scan(agent) -> None:
    ...
    concept(
        "Unlike Module 1 which queries GCP APIs, Module 2 analyzes local git repositories. "
...
# ---------------------------------------------------------------------------
# Section 9 — Dependency Analysis and GCP Mapping
# ---------------------------------------------------------------------------

def section_9_dependency_analysis(agent) -> None:
    header("SECTION 9 — Stack Analysis & GCP Service Mapping", "yellow")
    box(
        "From dependencies to GCP infrastructure requirements",
        "The agent reads dependency files, identifies libraries, and maps them "
        "to required GCP services (Cloud SQL, Memorystore, Cloud Storage, etc.).",
    )

    concept(
        "This is where repository analysis becomes infrastructure planning. "
        "The agent knows that 'pg' → Cloud SQL PostgreSQL, 'redis' → Memorystore, "
        "'google-cloud-storage' → S3/GCP SDK. It builds a complete infrastructure requirements list."
    )
...
3. Map dependencies to GCP infrastructure services
4. Tell me what GCP services each application needs"""
...
def section_11_full_workflow(agent) -> None:
    ...
    concept(
        ...
        "GCP infrastructure requirements. This output can be consumed by Module 1 "
        "(the GCP Infrastructure Agent) to check existing resources and identify gaps."
    )
...
3. For each application:
   ...
   - Map to GCP infrastructure requirements
4. Produce a comprehensive report with:
   - Repository overview
   - Application details (stack, dependencies, GCP needs)
   - Infrastructure summary (all GCP services needed)
...
def main() -> None:
  ...
  what applications exist, what technology stacks they use, and what GCP
  ...
  - Map dependencies to GCP services
  
  Framework: LangChain + LangGraph (vs Module 1's Google ADK)
""")
    ...
    print("""
  ✅ You've seen:
     • LangChain vs Google ADK framework comparison
  ...
  💡 Multi-Agent Future:
     Module 2 Agent (repo analysis) + Module 1 Agent (GCP infrastructure)
