...
Demonstrates Terraform generation agent, evaluation harnesses, routing agent, and
ISV integrations (Patronus AI, Deepchecks, Comet ML).

USAGE
-----
  # Recommended: mock mode (no GCP account or API keys needed)
...
def section_1_framework_consistency() -> None:
    ...
    box(
        ...
        "Both Module 2 (Repository Analysis) and Module 3 (Terraform Generation) use LangChain.\n"
  ...
  └─────────────────────────────────────────────────────────────────┘
              │
  ┌─────────────────────────────────────────────────────────────────┐
  │  MODULE 3: Terraform Generation Agent                                │
  │                                                                 │
  │    from langchain_google_genai import ChatGoogleGenerativeAI                        │
  │    from langgraph.prebuilt import create_react_agent           │
  │                                                                 │
  │    model = ChatGoogleGenerativeAI(model_id="claude-sonnet-4", ...)        │
  │    agent = create_react_agent(model, terraform_tools)                │
  │                                                                 │
  │    Tools: analyze_requirements, generate_terraform_module, validate   │
  └─────────────────────────────────────────────────────────────────┘
""")
...
# ---------------------------------------------------------------------------
# Section 2 — Terraform Generation
# ---------------------------------------------------------------------------

def section_2_terraform_generation() -> None:
    header("SECTION 2 — Terraform Code Generation", "cyan")
    
    user_says("Generate Terraform infrastructure for a simple web application with PostgreSQL database")
    
    print("\n  [Module 3 Agent] Analyzing requirements...")
    print("  [Module 3 Agent] Using LangGraph ReAct Agent")
    print("  [Model] Claude Sonnet 4 via Vertex AI")
    print("  [Tools] 5 Terraform generation tools\n")

    # Simulate agent execution by calling tool functions directly
    from module3.tools import terraform_tools

    print("  🔧 [Step 1] ACT → analyze_infrastructure_requirements(...)")
    print("  ✓  OBSERVE ← Requirements parsed: VPC, Cloud SQL PostgreSQL, Cloud Run\n")

    print("  🔧 [Step 2] ACT → generate_terraform_module(module_type='network', ...)")
    result = terraform_tools.generate_terraform_module.invoke({"module_type": "network", "parameters": json.dumps({"vpc_name": "main"})})
    vpc_data = json.loads(result)
    print("  ✓  OBSERVE ← Network module generated\n")

    print("  🔧 [Step 3] ACT → generate_terraform_module(module_type='cloud_sql', ...)")
    result = terraform_tools.generate_terraform_module.invoke({"module_type": "cloud_sql", "parameters": json.dumps({"database_version": "POSTGRES_15", "availability_type": "REGIONAL"})})
    rds_data = json.loads(result)
    print("  ✓  OBSERVE ← Cloud SQL module generated\n")

    print("  🔧 [Step 4] ACT → validate_terraform_syntax(...)")
    print("  ✓  OBSERVE ← Syntax validation: PASS\n")

    box("Generated Network Module (excerpt)", vpc_data["data"]["code"][:400] + "\n...")
    
    concept(
        "The agent decomposed the request into multiple tool calls: analyze requirements, "
        "generate network, generate Cloud SQL, validate syntax. Each module follows GCP best practices "
...
4. What GCP region should this be deployed to?
...
Please provide these details so I can generate the most appropriate Terraform stacks."""
    )
...
def section_4_complex_infrastructure() -> None:
    ...
    user_says(f"Generate Terraform for: {json.dumps(requirements, indent=2)}")
    ...
    stacks = [
        ...
        ("MemorystoreModule", "Redis instance with automatic failover"),
        ("PubSubModule", "Pub/Sub topics for inter-service communication"),
        ("CloudRunApiModule", "API service on Cloud Run"),
        ("CloudRunWorkerModule", "Worker service on Cloud Run"),
    ]
    
    try:
        from rich.table import Table
        table = Table(title="Generated Terraform Stacks")
...
3. Actionability: Sufficient detail for Terraform generation (0-100%)
...
• Deterministic judging (temperature=0.0)"""
    ...
    sample_terraform = '''resource "google_compute_network" "vpc"
...
'''
    ...
    criteria = {
        "syntax_correctness": "Valid Python Terraform syntax",
        "completeness": "All required resources included",
        "best_practices": "Follows GCP best practices",
        "security": "Proper security configurations",
    }
    
    print("\n  [LLM Judge] Evaluating Terraform code...")
    ...
    result = evaluate_with_llm_judge(
        task_description="Generate VPC Terraform stack",
        agent_output=sample_terraform,
        criteria=criteria,
        verbose=True,
    )
...
• Version comparison across agent releases"""
    ...
    custom_criterion = evaluator.create_custom_criteria(
        name="terraform_idempotency",
        description="Terraform code can be deployed multiple times without errors",
        scoring_rubric={
    ...
    result = evaluator.evaluate(
        task="Generate Cloud Run Terraform module",
        output="Sample Terraform code...",
...
• Anomaly detection"""
    ...
    sample_output = """This Terraform stack will definitely work in all regions and will never fail. 
    ...
    result = evaluator.detect_hallucinations(
        ...
        context="Generate Terraform stack for web application",
    )
...
4. deployment_monitoring → Future capability"""
    ...
    test_requests = [
        ...
        "Generate Terraform for a Node.js app with PostgreSQL",
...
4. Maximum 2 iterations to bound latency"""
    ...
    print("\n  [Module 3 Agent] Generating Terraform stack...")
...
MODULE 2 AGENT (analyze repository)
  ↓
MODULE 3 AGENT (generate Terraform infrastructure)
  ↓
...
RESULTS + METRICS"""
    ...
    print("    → Mapping to GCP services: Cloud Run, Cloud SQL, Memorystore\n")
    
    module2_output = {
        "applications": 1,
        "stack": "Node.js + Express",
        "gcp_requirements": ["Cloud Run", "Cloud SQL PostgreSQL", "Memorystore Redis", "VPC"],
    }
    
    # Step 3: Module 3
    print("  [Step 3] MODULE 3 AGENT (Terraform Generation)")
    print("    → Parsing Module 2 output")
    print("    → Generating VPC stack")
    print("    → Generating Cloud SQL module")
    print("    → Generating Memorystore module")
    print("    → Generating Cloud Run module")
    print("    → Validating syntax: PASS\n")
    
    # Step 4: Evaluation
    print("  [Step 4] EVALUATION SYSTEM")
    print("    → LLM-as-judge evaluation")
    print("    → Terraform syntax validation")
    ...
    print("  Terraform Generation: ✓ 4 stacks generated")
...
def main() -> None:
    ...
    header("MODULE 3 DEMO — EVALUATION & Terraform GENERATION", "bold cyan")
    print("""
  ...
  This demo covers:
    • Terraform infrastructure generation with LangChain
...
""")
    ...
    sections = {
        ...
        2: section_2_terraform_generation,
