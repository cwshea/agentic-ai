...
**Focus**: Side-by-side comparison of Module 1 (Google ADK) vs Module 2 (LangChain)

**What you'll see**:
- Code examples showing both frameworks
- Same concepts, different APIs
- Model interfaces: `Gemini via Vertex AI` vs `ChatGoogleGenerativeAI`
...
**What you'll see**:
- Module 1: `InMemorySessionService` (automatic, fixed window)
...
**Key Takeaway**: Unlike Module 1 which queries GCP APIs, Module 2 analyzes local git repositories to understand application structure.
...
### Section 6: Stack Analysis & GCP Service Mapping
...
- Maps libraries to GCP services:
  - `pg` → Cloud SQL PostgreSQL
  - `redis` → Memorystore Redis
  - `google-cloud-storage` → Cloud Storage
  - `express` → Cloud Run / Cloud Functions
- Complete infrastructure requirements list

**Key Takeaway**: This is where repository analysis becomes infrastructure planning. The agent translates code dependencies into GCP service requirements.
...
1. **Framework Agnostic**: Same concepts (prompts, tools, context) across Google ADK and LangChain
