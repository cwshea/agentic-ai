...
5. map_gcp_services          - Map dependencies to GCP services
"""
...
# ---------------------------------------------------------------------------
# Dependency to GCP Service Mapping
# ---------------------------------------------------------------------------

DEPENDENCY_TO_GCP_SERVICE = {
    # Databases
    "pg": {"service": "Cloud SQL", "engine": "PostgreSQL", "category": "database"},
    "postgres": {"service": "Cloud SQL", "engine": "PostgreSQL", "category": "database"},
    "psycopg2": {"service": "Cloud SQL", "engine": "PostgreSQL", "category": "database"},
    "mysql": {"service": "Cloud SQL", "engine": "MySQL", "category": "database"},
    "mysql2": {"service": "Cloud SQL", "engine": "MySQL", "category": "database"},
    "pymysql": {"service": "Cloud SQL", "engine": "MySQL", "category": "database"},
    "mongodb": {"service": "Firestore", "engine": "Document DB", "category": "database"},
    "pymongo": {"service": "Firestore", "engine": "Document DB", "category": "database"},

    # Caching
    "redis": {"service": "Memorystore", "engine": "Redis", "category": "cache"},
    "ioredis": {"service": "Memorystore", "engine": "Redis", "category": "cache"},
    "memcached": {"service": "Memorystore", "engine": "Memcached", "category": "cache"},

    # Storage
    "google-cloud-storage": {"service": "Cloud Storage", "engine": "Object Storage", "category": "storage"},
    "@google-cloud/storage": {"service": "Cloud Storage", "engine": "Object Storage", "category": "storage"},
    "gcloud": {"service": "Cloud Storage", "engine": "Object Storage", "category": "storage"},

    # Queuing
    "amqplib": {"service": "Cloud Pub/Sub", "engine": "Message Queue", "category": "queue"},
    "pika": {"service": "Cloud Pub/Sub", "engine": "Message Queue", "category": "queue"},
    "celery": {"service": "Cloud Tasks", "engine": "Task Queue", "category": "queue"},

    # Search
    "elasticsearch": {"service": "Elastic Cloud on GCP", "engine": "Elasticsearch", "category": "search"},
    "opensearch-py": {"service": "Elastic Cloud on GCP", "engine": "OpenSearch", "category": "search"},

    # Web frameworks (compute inference)
    "express": {"service": "Cloud Run / Cloud Functions", "engine": "Node.js", "category": "compute"},
    "fastapi": {"service": "Cloud Run / Cloud Functions", "engine": "Python", "category": "compute"},
    "flask": {"service": "Cloud Run / Cloud Functions", "engine": "Python", "category": "compute"},
    "django": {"service": "Cloud Run / Cloud Functions", "engine": "Python", "category": "compute"},
    "gin": {"service": "Cloud Run / Cloud Functions", "engine": "Go", "category": "compute"},
    "spring-boot": {"service": "Cloud Run / Cloud Functions", "engine": "Java", "category": "compute"},
}
...
# ---------------------------------------------------------------------------
# TOOL 5 — map_gcp_services
# ---------------------------------------------------------------------------

@tool
def map_gcp_services(dependencies: str) -> str:
    """
    Map application dependencies to required GCP services.

    Analyzes the dependency list and identifies which GCP services would be
    needed to run the application (Cloud SQL, Memorystore, Cloud Storage, etc.).
    ...
    -------
    str
        JSON with GCP service requirements mapped from dependencies.
    """
    try:
        deps_data = json.loads(dependencies).get("data", {})
        if "error" in deps_data:
            return _wrap(deps_data, "map_gcp_services")
        ...
        for dep in dep_list:
            dep_lower = dep.lower()
            for key, service_info in DEPENDENCY_TO_GCP_SERVICE.items():
                if key in dep_lower:
                    category = service_info["category"]
                    if category not in gcp_services:
                        gcp_services[category] = []
                    ...
                    if service_entry not in gcp_services[category]:
                        gcp_services[category].append(service_entry)
                        matched_dependencies.append(dep)

        if framework or language != "unknown":
            gcp_services["compute"] = [{
                "service": "Cloud Run or Cloud Functions",
                "engine": language,
                "framework": framework,
        ...
        if gcp_services:
            gcp_services["networking"] = [{
                "service": "VPC",
                "components": ["Subnets", "Firewall Rules", "Cloud NAT"],
                "note": "Network isolation and security",
            }]

            if framework:
                gcp_services["networking"].append({
        ...
        return _wrap({
            ...
            "summary": {
                "databases": [s["service"] for s in gcp_services.get("database", [])],
                "caching": [s["service"] for s in gcp_services.get("cache", [])],
                "storage": [s["service"] for s in gcp_services.get("storage", [])],
                "queuing": [s["service"] for s in gcp_services.get("queue", [])],
                "compute": [s["service"] for s in gcp_services.get("compute", [])],
            },
        }, "map_gcp_services")

    except Exception as exc:
        return _wrap({"error": str(exc)}, "map_gcp_services")
