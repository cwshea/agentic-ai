...
| **Actionability** | Sufficient detail for Terraform generation | 25% |
| **Safety Awareness** | Security issues flagged | 25% |

### Module 3: Terraform Generation

| Dimension | Description | Weight |
|-----------|-------------|--------|
| **Syntax Correctness** | Valid Python Terraform syntax | 40% |
| **Completeness** | All required resources included | 20% |
| **Best Practices** | Follows GCP and Terraform best practices | 20% |
...
result = evaluate_with_llm_judge(
    task_description="Generate VPC Terraform stack with Cloud SQL",
...
criterion = evaluator.create_custom_criteria(
    ...
    description="Terraform code can be deployed multiple times",
    scoring_rubric={
...
result = evaluator.detect_hallucinations(
    output=agent_output,
    context="Generate Terraform stack",
...
exp_id = tracker.start_experiment(
    ...
    tags=["module3", "terraform-generation"],
...
Automated Terraform code quality assessment:

```python
from module3.evaluators.terraform_evaluator import evaluate_terraform_code

result = evaluate_terraform_code(
    terraform_code=generated_code,
    expected_resources=["VPC", "Cloud SQL", "Cloud Run"],
)
...
Part of the AI Agent Learning Series on GCP.
